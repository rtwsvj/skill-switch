...
原生 macOS App(SwiftUI,壳外调 `skill-switch` CLI 取数据,核心引擎一行不动)。侧边栏七屏:
...
- **MCP**:列出各工具配置里的 MCP server(默认只列、绝不连接);点「扫描」弹确认框——如实告诉你将启动什么本地进程/请求什么地址,确认后才连,取回工具清单做安全体检;工具定义和上次不一样会用大白话提醒「和上次不一样了(需要你确认)」,看过没问题可点「重新接受」。
