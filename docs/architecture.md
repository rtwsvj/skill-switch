...
| `audit/config-discovery.ts` | Discovers and audits known agent config files (`settings.json`, `mcp.json`, `claude_desktop_config.json`) for secrets and dangerous MCP server patterns. |
| `audit/mcp-audit.ts` | Audits MCP server config JSON: detects `curl | sh`, reverse-shell patterns, unpinned `npx`/`uvx`/`bunx`, and hardcoded secrets in env values. |
...
| `doctor.ts` | Three-way reconciliation: declared (`skills.json`) × locked (`skills.lock.json`) × disk. Produces four drift kinds: `missing`, `content-drift`, `stale-lock`, `extra-locked`. Uses a hash cache (`doctor-hash-cache.ts`) to avoid redundant SHA-256 recomputation. Also reports bypasses (forced installs) and legacy skill names. |
...
| `supply-chain.ts` | Unpinned `npx`/`uvx`/`bunx`, `curl \| sh` installer |
