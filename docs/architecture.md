...
| `security/output-safety.ts` / `security/url-safety.ts` | Terminal-safe rendering/redaction and per-hop remote URL validation. Redirects reject HTTPS downgrade and strip credentials on cross-origin hops. Literal and DNS-resolved private/special IPs are rejected; DNS rebinding between policy lookup and the HTTP client's lookup remains a documented residual risk. |
