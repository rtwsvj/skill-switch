...
## 一键接入(`skill-switch ci`)

不想手写 workflow 文件?在仓库根执行:

```bash
skill-switch ci                        # 生成 sarif 格式 workflow
skill-switch ci --format github        # PR 内联注解格式(无需 code-scanning 权限)
skill-switch ci --baseline             # 同时写入 finding 基线,CI 只对新 finding 失败
skill-switch ci --pin v0.8.0           # 固定 action 版本
```

命令会自动创建 `.github/workflows/skill-switch.yml`;`--baseline` 同时写入
`.skill-switch-baseline.json` 并在工作流 `args` 里自动加 `--baseline` 参数。
`--force` 可覆盖已存在的文件;`--json` 输出机器可读摘要。
