...
| `exfiltration/taint-source-to-sink` | high | taint 数据流：单文件内读敏感源（env/凭据/历史/浏览器·钱包）后近距离外发（curl 上传/nc//dev/tcp/外渗端点/base64 管道/scp），source→sink 关联成链 |
...
声明为文本（`.md`/`.txt`/`.json` 等）的文件却以可执行/归档格式的魔数开头——典型的"把二进制藏成技能文件"伪装。魔数表自写自公开文件格式规范（file(1) magic / 维基 List of file signatures）。

| ruleId | 严重度 | 抓什么 |
|--------|--------|--------|
| `masquerade/binary-magic-bytes` | critical | 文件起始命中可执行/归档魔数：PE（`MZ`）、PDF（`%PDF`）、ZIP/JAR/Office（`PK\x03\x04`）、RAR（`Rar!`）、Wasm（`\x00asm`）、ELF（`\x7FELF`）、7z、gzip 等（精确前缀匹配，正文提及魔数名不误报） |
| `masquerade/binary-lossy-head` | high | 文件以连续 U+FFFD 开头——utf8 读取后坍缩的二进制魔数（Mach-O `0xFEEDFACE`/`0xFEEDFACF`、fat binary、Java class `0xCAFEBABE`）的有损启发式检测 |
...
`audit --configs --config-baseline <file>` 将当前 MCP server 配置与快照基线对比，检测潜在的 rug-pull 或被篡改迹象。
...
`audit --configs --config-baseline <file>` 将当前 settings 文件的安全结构（hooks/permissions/auto-approve）与快照基线对比，检测潜在的 rug-pull 或被篡改迹象。

| ruleId | 严重度 | 抓什么 |
|--------|--------|--------|
| `settings/config-changed` | high | settings 文件的 hooks/permissions/auto-approve 自基线起已变更——可能是 rug-pull 或被篡改，请复核后重新运行 `--write-config-baseline` |
| `settings/config-added` | medium | 发现基线中未记录的新 settings 文件——请确认内容可信后更新基线 |
