...
`audit --configs --mcp-baseline <file>` 将当前 MCP server 配置与快照基线对比，检测潜在的 rug-pull 或被篡改迹象。

| ruleId | 严重度 | 抓什么 |
|--------|--------|--------|
| `mcp/server-config-changed` | high | MCP server 的 `command`/`args`/`url` 自基线起已变更——可能是 rug-pull 或被篡改，请复核后重新运行 `--write-mcp-baseline` |
