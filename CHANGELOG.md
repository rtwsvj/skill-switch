...
### 新增 Added
- **WAL 写前意图日志(崩溃恢复)**:`install` / `toggle` / `remove` / CLI `sync` 在权威写前落写前意图日志(journal);进程崩溃(`SIGKILL`/OOM)后,**下一次任何写操作**获锁时自动回滚到操作前或前滚完成已提交事务。恢复引擎**不信任日志内路径**(结构化目标 + 边界校验 + 快照预验);诚实边界:承诺进程崩溃级、**不承诺断电**。`doctor` 可见待恢复/损坏 journal 并可用 `doctor --fix` 立即恢复(只读路径绝不自动 recover)。
...
- **审计引擎:`obfuscation/unicode-supplementary-variation-selectors`(补充变体选择符走私检测,high)**:闭合既有 invisible-chars 规则未覆盖的盲区——Unicode 补充变体选择符 `U+E0100–U+E01EF`(VS17–VS256)。威胁模型与 `obfuscation/unicode-tag-chars` 平行:这些字符对人眼不可见、LLM 在 token 层可读,攻击者可借此走私数据/隐藏指令。**关键护栏:绝不检测 `U+FE00–U+FE0F`**(emoji 变体选择符 `️`/`⚠️`/`❤️` 等 + 常用 CJK 表意字符标准化 IVS 的合法用途海量)。FP 风险评估:`VS17–VS256` 仅服务于 CJK 扩展 B+ (`U+20000+`) 在 IVD 中注册的少数 IVS 序列,skill 类场景近乎为零;emoji 全部走 VS16,常用 CJK 走 VS1–VS16,均不命中。命中样本 + emoji/CJK 零误报样本(⚠️/✅/❤️/🏳️‍🌈/CJK IVS)入 A5 语料与本规则 FP 矩阵。零新依赖(纯字符区间正则)。
- **macOS App「MCP」屏(里程碑 6)**:让非命令行用户也能用 `mcp-scan`——侧边栏新增 MCP 屏(SF Symbol `server.rack`),进屏默认 `mcp-scan --json` list 模式只列不连;每行卡片显示 transport 徽章(本地进程/远程 endpoint)、配置文件路径、CLI 给的「将执行」一行;「扫描」按钮走**原生确认弹窗**,stdio 明说"将启动本地进程:<command>",http 明说"将请求 <url>",后半句共用大白话「skill-switch 只读取工具清单,绝不调用任何工具」;确认后 GUI 侧追加 `--yes` 调 CLI,渲染结果:基线状态(已建立/已比对/已重置)、工具数、finding 行(severity 徽章 + ruleId + message)、rug-pull findings 单独一组(大白话标题「和上次不一样了(需要你确认)」+ removedTools);连接失败时 `error.message` 原样进行内红条提示不弹窗。有 rug-pull 时才出现的「重新接受」按钮 → 二次确认弹窗(说明会把当前清单写为新基线)→ `--reset-baseline` 刷新该行。四语言 i18n 齐全(`nav.mcp` + `mcpscan.*` 33 个 key,中日英西多参数文案一律 `%1$@/%2$@` 位置化占位符)。GUI 自身不实现任何 CLI 没有的连接逻辑——只是 CLI 的壳。
