...
async function installFromSourceUnlocked(
  ...
  try {
    ...
    if (existsSync(skillsDir) && (await readdir(skillsDir)).length > 0) {
      ...
      walSnapshots.push({
        path: snap.path,
        scope: { kind: 'agent-skills-root', agent: options.agent },
      });
    }

    // WAL preState:在任何权威写之前,记下声明/锁原文与将被触碰的目标(结构化:
    // agent+skill 名,恢复时重新推导路径);copy 模式下已存在的 store 目录会被
    // rm+重写,先逐个快照以便回滚铺回。
    const walTargets: JournalManagedTarget[] = [];
    const walStoreTargets: JournalManagedTarget[] = [];
    for (const dir of skillDirs) {
      const name = basename(dir);
      const target = join(skillsDir, name);
      walTargets.push({ agent: options.agent, name, existedBefore: existsSync(target) });
      if (options.mode === 'copy') {
        const storeTarget = durableCopySource(options.home, options.agent, name);
        const existedBefore = existsSync(storeTarget);
        walStoreTargets.push({ agent: options.agent, name, existedBefore });
        if (existedBefore) {
          ...
          walSnapshots.push({
            path: snap.path,
            scope: { kind: 'store-skill', agent: options.agent, name },
          });
        }
      }
    }
    ...
    try {
      ...
      // 就地恢复:进程还活着时不必等下一次写操作——锁仍持有,立刻按 journal 回滚。
      // 恢复失败(如快照损坏)时保留 journal,下一次获锁操作会重试恢复。
      try {
        await recoverPendingJournal(options.home, {
          log: (message) => console.error(message),
        });
      } catch (recoveryError) {
        throw new AggregateError(
          [error, recoveryError],
          'install 失败且就地恢复未完成;写操作日志已保留,下次任意写操作将自动重试恢复',
          { cause: error },
        );
      }
      throw error;
